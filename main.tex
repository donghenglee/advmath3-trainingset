...
%output copyable code snippet: 简体字版本latex code的copyable code snippet,注意一定要简体字,以\question开始然后才\begin{solution},请完全按照原文and give solutions as well,but the ques also in chinese,有小题的话用\question \begin{parts}然后一小题\part accompanied by 一个\begin{solution},use \subpart or \subsubpart when necessary, dx之前要用\,whenever necessary,用\[...\]或\begin{align*}或$...$但不要用$$...$$,\[...\]里不能放\begin{align*},\[然后下一行content然后下一行\],用\frac{}{}不要a/b,中文字不可以在\[...\]里面,不要用\text{中文字},不要用\textbf{},\boxed及\ans,不要用\overline{},句号。不要在\[...\]里面,如果有\frac{}{}在brackets里要\left(...\right),把这些⑦变为(7)
...
%singapore exam
%hongkong exam: hkla
%msia stpm old questions
%统计学题目https://zhuanlan.zhihu.com/p/105360052
%概率论与数理统计教程（第2版）茆诗松
%上海师范大学-数理逻辑
...
\definecolor{jeeRed}{RGB}{219, 68, 85}
\definecolor{gaokaoBlue}{RGB}{42, 122, 228}
\definecolor{gridGray}{RGB}{200, 200, 200}
\definecolor{labelDark}{RGB}{50, 50, 50}

\newcommand{\radarChart}[3]{%
    \begin{tikzpicture}[scale=0.85]
        \def\scores{{#3}}
        
        % 1. Grid: Concentric Octagons (Levels 1 to 5)
        \foreach \level in {1,...,5} {
            \draw[color=gridGray, dashed, line width=0.4pt] 
                (0*45: \level*0.7) 
                \foreach \i in {1,...,7} { -- (\i*45: \level*0.7) } -- cycle;
            \node[color=gray, font=\tiny, anchor=south west] at (45:\level*0.7) {\level};
        }

        % 2. Axes & Metric Labels (Direct iteration over strings)
        \foreach \label [count=\i from 0] in {Scope, Depth, Trickiness, Innovation, Pacing, Load, Intertwinability, Error Margin} {
            \pgfmathsetmacro{\ang}{\i*45}
            \pgfmathsetmacro{\anch}{180 + \i*45}
            \draw[color=gridGray, line width=0.5pt] (0,0) -- (\ang: 3.5);
            \node[font=\small\bfseries, color=labelDark, anchor=\anch] 
                at (\ang: 3.85) {\label};
        }

        % 3. Extract scores into points
        \pgfmathparse{\scores[0]}\let\sA\pgfmathresult
        \pgfmathparse{\scores[1]}\let\sB\pgfmathresult
        \pgfmathparse{\scores[2]}\let\sC\pgfmathresult
        \pgfmathparse{\scores[3]}\let\sD\pgfmathresult
        \pgfmathparse{\scores[4]}\let\sE\pgfmathresult
        \pgfmathparse{\scores[5]}\let\sF\pgfmathresult
        \pgfmathparse{\scores[6]}\let\sG\pgfmathresult
        \pgfmathparse{\scores[7]}\let\sH\pgfmathresult

        % 4. Draw Data Polygon
        \draw[line width=1.4pt, color=#2, fill=#2!20] 
            (0*45: \sA*0.7) -- 
            (1*45: \sB*0.7) -- 
            (2*45: \sC*0.7) -- 
            (3*45: \sD*0.7) -- 
        ...
        % 5. Data Vertex Dots
        \foreach \i in {0,...,7} {
            \pgfmathparse{\scores[\i]}
            \fill[color=#2] (\i*45: \pgfmathresult*0.7) circle (2.5pt);
        }

        % 6. Chart Title
        \node[font=\large\bfseries, color=labelDark] at (0, -4.8) {#1};
    \end{tikzpicture}%
}
...
\begin{itemize}
    ...
    \item \href{https://mathrevise.weebly.com/27511236223543038988m2---pure-maths.html}{Hong Kong Advanced Level Examination (HKALE) - Pure Mathematics}
    \item \href{https://theannexeproject.com/model_solution_category/gce-a-level-h2-math-solutions/}{GCE A Level H2 Math}
    \item \href{https://chuoffice.blogspot.com/}{STPM MATHEMATICS - Chu Maths T Office}
